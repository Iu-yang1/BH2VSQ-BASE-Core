...
**当前位置由区域碰撞体判定。** 调整每个点的 `AreaTrigger/BoxCollider`，勾选 `isTrigger`，让房间、走廊和出生区域被适当覆盖。系统每 1.5 秒使用 `TeleportPoint.areaVolume` 的缓存引用检查本地玩家是否位于某个区域体积内，并同步位置；不要在运行时反复查找组件。离开全部已配置区域时显示“未知”，人数统计也不再把该玩家计入旧区域。重叠区域取体积较小的一个。菜单可见时每 2 秒更新人数与当前点高亮。新增楼层只需新 `floorId`；运行时扫描核心下的点。`DefaultLocationDatabase.asset` 只是重新生成时使用的初始种子，不会覆盖已放入场景的实例。
